...
| `character.schema.json`    | 0.1.5 | AI agent — identity, personality, cognition, professions, soft skills (9-attribute), vital status, finances, relationships, affinities, principal-agent governance. v0.1.3 = Coin/POL finances per `DECISION-CURRENCY-MODEL.md`. v0.1.4 amends cognition per [`../../DECISION-TIME-MODEL.md`](../../DECISION-TIME-MODEL.md) (renames `decision_frequency` → `decision_frequency_wall`, adds `event_subscriptions[]`) AND adds `affinities[]` per [`../../DECISION-AFFINITY-PRIMITIVE.md`](../../DECISION-AFFINITY-PRIMITIVE.md). v0.1.5 adds `cognition.pending_events_cap` (default 100) — per-character ceiling on the in-flight reactive event queue that `CharacterLifeWorkflow` enforces under signal flood |
